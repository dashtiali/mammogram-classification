...
# an ULBP geometry into one feature vector and export to csv
def SaveFeaturesToCSV(datasetPath, dataset):
    for setName in dataset:
        ...
        for geo in range(7):
            print(geo)
            dim0_bin = np.genfromtxt(f'{setPath}\\G{geo}\\dim0_bin.csv', delimiter=',')
            dim0_stats = np.genfromtxt(f'{setPath}\\G{geo}\\dim0_stats.csv', delimiter=',')

            dim1_bin = np.genfromtxt(f'{setPath}\\G{geo}\\dim1_bin.csv', delimiter=',')
            dim1_stats = np.genfromtxt(f'{setPath}\\G{geo}\\dim1_stats.csv', delimiter=',')

            dim0_persims = np.genfromtxt(f'{setPath}\\G{geo}\\dim0_persims.csv', delimiter=',')
            dim1_persims = np.genfromtxt(f'{setPath}\\G{geo}\\dim1_persims.csv', delimiter=',')
            
            dim0_perland = np.genfromtxt(f'{setPath}\\G{geo}\\dim0_perland.csv', delimiter=',')
            dim1_perland = np.genfromtxt(f'{setPath}\\G{geo}\\dim1_perland.csv', delimiter=',')
